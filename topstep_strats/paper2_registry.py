# CHANGE_SUMMARY
# 2026-08-19  coder
#   - Created topstep_strats/paper2_registry.py as the Paper-2 analog of
#     paper1_registry.py.
#   - Wraps topstep_strats.strategies.paper2_matrix, which holds the exact
#     108-row Paper-2 strategy matrix (9 engines x 3 sessions x 2 instruments x
#     2 risk profiles).
#   - Keeps INSTRUMENT_CONFIG, SESSION_CONFIG, CHUNKS, default_data_path(), and
#     make_strategy_key() so runners and the GitHub Actions workflow import a
#     single canonical source.
# WHY: The Paper-2 sweep must evaluate the 9 architectural engines from the
#      user's second research report, not the Paper-1 blueprints.
...
def get_strategy_config(strategy_id: int) -> Dict[str, Any]:
    """Return the Paper-2 configuration for a zero-based strategy ID.

    IDs 0-107 map to matrix rows 'P2_001'-'P2_108'.
    """
    ...
    matrix_id = f"P2_{strategy_id + 1:03d}"
...
def make_strategy_key(strategy_id: int, instrument: str, session: str) -> str:
    ...
    return f"paper2_{strategy_id:03d}_{blueprint}_{instrument}_{session}"
