...
#     aggregation step via aggregate_paper1.py.
# WHY: Paper 1 is a structured sweep; this script turns the Cartesian product
...
from topstep_strats.paper1_registry import (
    CHUNKS,
    INSTRUMENT_CONFIG,
    SESSION_CONFIG,
    default_data_path,
...
def _run_chunk(args: Tuple[Path, int, str, str, str, str, str, Path]) -> Tuple[str, ...]:
    ...
    cmd = [
        sys.executable,
        str(project_root / "scripts" / "run_paper1_chunk.py"),
...
def _build_tasks(args) -> List[Tuple[Path, int, str, str, str, str, str, Path]]:
    ...
    from topstep_strats.paper1_registry import N_STRATEGIES
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Run a slice of the Paper 1 strategy matrix across instruments, sessions, and date chunks."
    )
    parser.add_argument(
        "--strategy-id-start",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Run aggregate_paper1.py after chunks finish (default: True)",
    )
    parser.add_argument(
        "--no-aggregate",
        action="store_true",
    ...
    print(
        f"[run_paper1_sweep] {len(tasks)} tasks: strategy-ids "
    ...
    if args.aggregate and not args.no_aggregate:
        ...
        agg_cmd = [
            sys.executable,
            str(_PROJECT_ROOT / "scripts" / "aggregate_paper1.py"),
            "--input", str(args.output_dir),
            "--output", str(aggregate_dir),
        ]
        print(f"[run_paper1_sweep] aggregating: {' '.join(agg_cmd)}")
        subprocess.run(agg_cmd, cwd=_PROJECT_ROOT, check=True)
        print(f"[run_paper1_sweep] aggregate report: {aggregate_dir / 'final_report.json'}")

    print(f"[run_paper1_sweep] done. failures={failures}/{len(tasks)}")
