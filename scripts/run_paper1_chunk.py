# CHANGE_SUMMARY
# 2026-08-19  kilo
#   - Created run_paper1_chunk.py, the Paper 1 per-cell entry point.
#   - Takes strategy-id, instrument, session, start/end dates, output path, and
#     an optional data path.
#   - Resolves the strategy configuration from topstep_strats.paper1_registry,
...
# WHY: Paper 1 needs a matrix-style runner that crosses 100 strategy variants
...
from topstep_strats.strategies.paper1_matrix import generate_signals
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Backtest one Paper 1 cell (strategy-id x instrument x session x chunk).")
    parser.add_argument("--strategy-id", type=int, required=True, help="Index into the Paper 1 strategy registry")
    parser.add_argument(
        "--instrument",
        required=True,
    ...
    if data_path is None:
        from topstep_strats.paper1_registry import default_data_path
    ...
    print(f"[run_paper1_chunk] wrote {output_path}")
