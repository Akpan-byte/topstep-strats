# CHANGE_SUMMARY
# 2026-08-19  coder
#   - Created scripts/aggregate_paper1.py as the paper1_sweep.yml finalizer.
...
#   - Adds per-instrument aggregation because each paper1 strategy ID is run on
#     ES/NQ/YM.
# 2026-08-19  coder
#   - Fixed instrument extraction: run_paper1_chunk.py stores the instrument in
#     ``params.instrument``, not at the record top level, so the by-instrument
#     breakdown now groups correctly.
# WHY: aggregate.py already knows how to combine raw/topstep reports by strategy;
#      paper1 needs the same plus instrument-level breakdowns.
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Aggregate paper1_sweep chunk JSONs into a final report."
    )
    ...
    print(f"[aggregate_paper1] wrote {report_path}")
