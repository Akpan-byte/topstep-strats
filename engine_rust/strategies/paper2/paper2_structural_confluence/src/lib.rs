// CHANGE_SUMMARY
// 2026-08-20  kilo
//   - Rust C ABI plugin for the Structural Confluence (Unicorn) Matrix engine.
//   - Mirrors topstep_strats/strategies/paper2_matrix.py::_structural_confluence_unicorn.
// WHY: Fast, language-agnostic Paper-2 engine plugin.

use engine_abi::{Bar, Signal, Strategy, StrategyVTable};
use paper2_indicators::{
    adx, atr, cfg_bool, cfg_f64, cfg_usize, date_day, ema, evaluate_filter, in_session,
    parse_time_to_seconds, volume_sma, vwap,
};
...
impl From<&Value> for Params {
    fn from(cfg: &Value) -> Self {
        Params {
            ema_fast: cfg_usize(cfg, "ema_fast", 9),
            ema_slow: cfg_usize(cfg, "ema_slow", 21),
            adx_threshold: cfg_f64(cfg, "adx_threshold", 30.0),
            volume_mult: cfg_f64(cfg, "volume_mult", 1.5),
...
impl State {
    ...
    fn prepare(&mut self, bars: &[Bar]) {
        ...
        self.ema_fast = ema(&closes, self.params.ema_fast);
        self.ema_slow = ema(&closes, self.params.ema_slow);
...
impl Strategy for State {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Option<Signal> {
        ...
        let ema_f = self.ema_fast[i]?;
        let ema_s = self.ema_slow[i]?;
        let adx = self.adx[i];
        ...
        let bar_range = h - l;
        let near_high = bar_range > 0.0 && (h - c) <= 0.2 * bar_range;
        let near_low = bar_range > 0.0 && (c - l) <= 0.2 * bar_range;
        let volume_ok = vol > self.params.volume_mult * vol_sma;

        if c > ema_f && ema_f > ema_s && c > vwap && adx >= self.params.adx_threshold && near_high && volume_ok {
        ...
        if c < ema_f && ema_f < ema_s && c < vwap && adx >= self.params.adx_threshold && near_low && volume_ok {
            if let Some(sig) = self.make_signal(i, -1, a) {
                self.last_date = d;
                return Some(sig);
            }
...
mod tests {
    ...
    #[test]
    fn test_unicorn_long_signal() {
        let base_ts = 1_756_000_200_000_000_000i64;
        let mut bars = Vec::new();
        for i in 0..80 {
            let c = 100.0 + i as f64 * 0.4;
            let h = c + 0.1; // close near high
            let l = c - 0.5;
            let v = if i > 60 { 50_000u64 } else { 1_000u64 }; // volume spike late
            bars.push(bar(base_ts + i * 60_000_000_000, c, h, l, c, v));
        }
        let mut state = State::new(Params::default());
        state.prepare(&bars);
        let mut first_long: Option<i64> = None;
        for b in &bars {
            if let Some(sig) = state.on_bar(b) {
                if sig.direction == 1 && first_long.is_none() {
                    first_long = Some(b.timestamp_ns);
                }
            }
        }
        assert!(first_long.is_some(), "expected a long unicorn signal");
    }
