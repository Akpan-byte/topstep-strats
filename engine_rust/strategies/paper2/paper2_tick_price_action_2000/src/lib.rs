// CHANGE_SUMMARY
// 2026-08-20  kilo
//   - Rust C ABI plugin for the 2000-Tick Price Action Master engine.
//   - Mirrors topstep_strats/strategies/paper2_matrix.py::_tick_price_action_2000.
...
impl From<&Value> for Params {
    fn from(cfg: &Value) -> Self {
        Params {
            consolidation_bars: cfg_usize(cfg, "consolidation_bars", 5),
            consolidation_range_atr: cfg_f64(cfg, "consolidation_range_atr", 0.5),
            breakout_threshold_atr: cfg_f64(cfg, "breakout_threshold_atr", 1.0),
...
impl Strategy for State {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Option<Signal> {
        ...
        let cons = self.params.consolidation_bars;
        if i < cons + 1 {
            return None;
        }

        let max_range = self.bars[i - cons..i]
            .iter()
            .map(|b| b.high - b.low)
            .fold(0.0f64, |m: f64, r| m.max(r));
        if max_range > self.params.consolidation_range_atr * a {
            return None;
        }
        ...
        let bar_range = h - l;
        if bar_range < self.params.breakout_threshold_atr * a {
            return None;
        }

        let window_high = self.bars[i - cons..i].iter().map(|b| b.high).fold(f64::NEG_INFINITY, |m, x| m.max(x));
        let window_low = self.bars[i - cons..i].iter().map(|b| b.low).fold(f64::INFINITY, |m, x| m.min(x));
...
mod tests {
    ...
    #[test]
    fn test_tick_price_action_generates_signals() {
        let base_ts = 1_756_000_200_000_000_000i64;
        let mut bars = Vec::new();
        for i in 0..60 {
            let c = 100.0 + i as f64 * 0.05;
            bars.push(bar(base_ts + i * 60_000_000_000, c, c + 0.2, c - 0.2, c, 1000));
        }
        // Breakout bar
        bars[40].close = 103.0;
        bars[40].high = 103.5;
