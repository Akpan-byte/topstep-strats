...
# 2026-08-21  kilo
#   - Added --paper asian_paper support for the 108-strategy Asian-paper matrix
#     (9 engines x ES/NQ/YM x Early/Late Asian x Hyper-Scalp/Mean-Reversion).
#   - Added OUTPUT_COLUMNS_ASIAN_PAPER, _asian_paper_tasks, _build_asian_paper_cfg,
#     and _run_asian_paper_group.  Each strategy ID carries its own TP/SL and
#     runs a single baseline exit pass.
# WHY: User requested a full Topstep-modeled backtest of the Asian-paper matrix
#      described in asian_paper.txt.

#!/usr/bin/env python3
"""Rust/PyO3 sweep orchestrator for Paper-1, Paper-2, and Asian-paper matrices.
...
Run the Asian-paper matrix:
    python engine_rust/scripts/sweep_engine.py --paper asian_paper --workers 16 --output-dir /tmp/asian_paper_rust_sweep
...
from topstep_strats.strategies.asian_paper_matrix import (
    ...
    get_strategy_config as get_asian_paper_config,
    list_strategy_ids as list_asian_paper_ids,
    _atr as _asian_paper_atr,
)
...
ASIAN_PAPER_SESSIONS = {
    "EarlyAsian": ("18:00", "22:00"),
    "LateAsian": ("22:00", "02:30"),
}
...
# ---------------------------------------------------------------------------
# Asian-paper combo runner
# ---------------------------------------------------------------------------
def _build_asian_paper_cfg(sid: str, start: str, end: str) -> Dict[str, Any]:
    """Build config for one Asian-paper strategy ID.

    The 108-row matrix already encodes instrument, session, engine, and target
    style, so this only pins the date range and ensures session windows are
    derived from the matrix.
    """
    cfg = get_asian_paper_config(sid)
    cfg.update(
        {
            "session_start": ASIAN_PAPER_SESSIONS[cfg["session"]][0],
            "session_end": ASIAN_PAPER_SESSIONS[cfg["session"]][1],
            "session_only": True,
            "one_trade_per_day": True,
            "stop_first": True,
        }
...
def _run_asian_paper_group(
    args: Tuple[str, str, str, str, str, bool, str],
) -> List[Dict[str, Any]]:
    sid, start, end, data_dir, topstep_enabled, account_size = args
    ...
    try:
        ...
        print(f"  Asian-paper signal generation failed: {exc}", file=sys.stderr)
        return []
...
def _asian_paper_tasks(args) -> List[Tuple[str, str, str, str, bool, str]]:
    sids = list_asian_paper_ids()[args.id_start : args.id_end]
    start = args.start_date
    end = args.end_date
    return [
        (sid, start, end, args.data_dir, args.topstep, args.account_size)
        for sid in sids
    ]
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--paper", required=True, choices=["paper1", "paper2", "portfolio1", "asian_paper"], help="Which matrix to sweep")
...
def main(argv=None) -> int:
    ...
    if args.paper == "portfolio1":
        ...
        columns = OUTPUT_COLUMNS_ASIAN_PAPER
    else:
    ...
    if args.paper == "paper1":
        ...
        tasks = _asian_paper_tasks(args)
        worker_fn = _run_asian_paper_group
    else:
